...
<?php
	echo "<a href='/test.php'>Back to Group </a>";
        echo "<a href='/Inventory.php?ID=" .$_GET["PL_ID"] . "'>Back to Inventory</a>";
...
$sql = "SELECT *,t1.ID as ID FROM Items as t1 
	where t1.ID = ". $_GET["ID"];
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
	echo "<div class='TitleBar'>" . $row["Name"] . "</div><div class='Skills'>" . $row["Description"] . "</div>";
