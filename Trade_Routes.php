...
LEFT JOIN  `Dmn_City` AS t3 ON t1.CityB = t3.Dmn_Cityid";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
	while($row = $result->fetch_assoc()) {
		echo "<div id=".$row["Dmn_Cityid"]." style='display:inline;".$paddingValue."'><a href='Trade_Routes_Edit.php?CityA=".$row["CityA"]."&CityB=".$row["CityB"]."'>".$row["CityAName"]. " to " .$row["CityBName"]. "</a> " .$row["Distance"]. " " .$row["Difficulty"]. "</div>";
		$sql = "SELECT *
			FROM  `Dmn_TradeRoute` AS t1
