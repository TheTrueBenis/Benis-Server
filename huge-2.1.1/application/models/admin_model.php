...
class AdminModel
{
	...
	/**
     * Gets an array that contains all the Characters in the database. The array's keys are the character ids.
     * Each array element is an object, containing a specific Character's data.
     * @return array The profiles of all users
     */
    public function getAllCharacters()
    {
        $sth = $this->db->prepare("SELECT * FROM Players t1 
		left join Class t2 on t1.Class = t2.ID	
		left join Race t3 on t1.Race = t3.ID 
		left join users t4 on t1.User_ID = t4.user_id 
		");
        $sth->execute();

        $all_character_profiles = array();

        foreach ($sth->fetchAll() as $char) {
            // a new object for every user. This is eventually not really optimal when it comes
            // to performance, but it fits the view style better
            $all_character_profiles[$char->ID] = new stdClass();
            $all_character_profiles[$char->ID]->user_name = $char->user_name;
            $all_character_profiles[$char->ID]->Character_name = $char->Name;
			$all_character_profiles[$char->ID]->XP = $char->XP;
			$all_character_profiles[$char->ID]->Class = $char->Class_Name;
			$all_character_profiles[$char->ID]->Race = $char->Race;
            //$all_character_profiles[$char->ID]->user_email = $user->user_email;

        }
        return $all_character_profiles;
    }
}
