...
class NoteModel
{
    ...
    {
        $sql = "SELECT user_id, note_id, note_text, note_description FROM notes WHERE user_id = :user_id AND note_id = :note_id";
        $query = $this->db->prepare($sql);
     ...
     */
    public function editSave($note_id, $note_text, $note_description)
    {
        // clean the input to prevent for example javascript within the notes.
        $note_text = strip_tags($note_text);
		$note_description = strip_tags($note_description);
        $sql = "UPDATE notes SET note_text = :note_text,note_description =:note_description WHERE note_id = :note_id AND user_id = :user_id";
        $query = $this->db->prepare($sql);
        $query->execute(array(':note_id' => $note_id, ':note_text' => $note_text, ':user_id' => $_SESSION['user_id'], ':note_description' => $note_description));
